...
- 主流程：网站登录后在同步面板点击“扫码连接”，用微信扫描二维码授权当前账号。
- 备用流程：点击“生成绑定码”，微信发 `绑定 TD-xxxxxxxxxx` 后绑定当前 Todo 账号。
- 未绑定微信会话的普通消息会静默，不会打扰这个微信号的正常聊天。
- 微信可发 `任务列表`、`今天有什么`、`完成 xxx`、`删除 xxx`、`明天10点提醒我xxx`。
- 云服务器上的 iLink poller 会扫描授权用户的微信消息并发回回复。
- 备用 OpenClaw 服务器流程会启动本地 Todo API，OpenClaw 收到微信消息后调用本机 `127.0.0.1`。
- 云服务器上的定时器会扫描提醒，到点后通过对应微信通道发回消息。

完整服务器安装和上线流程见：

```text
docs/openclaw-server.md
docs/wechat-ilink-scan.md
```
